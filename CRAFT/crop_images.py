...
def generate_words(image_name, score_bbox, image):
    ...
    for num in range(num_bboxes):
        ...
        if bbox_coords != ['{}']:
            l_t = float(bbox_coords[0].strip(
                ' array([').strip(']').split(',')[0])
            t_l = float(bbox_coords[0].strip(
                ' array([').strip(']').split(',')[1])
            ...
            b_l = float(bbox_coords[3].strip(
                ' [').strip(']').split(',')[1].strip(']'))
            pts = np.array([[int(l_t), int(t_l)], [int(r_t), int(t_r)], [
                           int(r_b), int(b_r)], [int(l_b), int(b_l)]])
