...
def getDetBoxes(textmap, linkmap, text_threshold, link_threshold, low_text, poly=False):
    boxes, labels, mapper, det_scores = getDetBoxes_core(
...
def adjustResultCoordinates(polys, ratio_w, ratio_h, ratio_net=2):
    adjusted_polys = []
    max_vertices = 8  # Change this to the desired number of vertices

    for poly in polys:
        if poly is not None:
            # Ensure all polygons have the same number of vertices (pad or truncate as needed)
            num_vertices = len(poly)
            if num_vertices < max_vertices:
                # Pad with the last vertex to match the desired number of vertices
                padding = [poly[-1]] * (max_vertices - num_vertices)
                poly = np.vstack((poly, padding))
            elif num_vertices > max_vertices:
                # Truncate to the desired number of vertices
                poly = poly[:max_vertices]

            # Convert the polygon to a NumPy array and adjust coordinates
            poly = np.array(poly)
